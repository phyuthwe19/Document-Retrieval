...
<html>
    ...
    <?php
        ...
        if(isset($_POST["search"])) {            
            $monthyear = $_POST["monthYearPicker"];
            ...
            $statement = $connection->prepare("SELECT out_id,out_date,to_dept,letter_no,description,remark,file_name,categoryname FROM outgoing_message om, category c WHERE om.categoryid = c.categoryid AND MONTH(om.out_date) = ? AND YEAR(om.out_date) = ?");
            $statement->bind_param("is", $month_no, $year);
            $statement->execute();
